...
1. **工作台**：快捷创建、资产总览、任务概览、最近内容
2. **课程创建 4 步向导**：设定课程 → 设计冒险 → 选择路径 → 添加魔法
3. **课程创作工作流**：课程地图 → 教案设计（4E）→ PPT 课件
4. **AI 素材生成**：图片（13 类）、视频（IP 体能 / 多人叙事）、音频（6 类 PPT 素材）
5. **素材库**：图片库 / 音频库 / 视频库 / 教材资源
6. **绘本制作**：知识库上传 → RAG 检索 → 故事规划 → 分页文案 → 逐页配图
7. **任务中心**：队列轮询（5s）、失败重试、结果插入画布
8. **权限体系**：super_admin / org_admin / research_leader / creator / viewer
