 ...
 * @param {string} userId - 用户ID
 * @param {string} organizationId - 组织ID
 * @param {string} previousSceneImage - 前一个分镜的图片URL（用于保持风格一致性）
 * @param {string} stylePrompt - 风格提示词（用于保持所有分镜风格一致）
 * @returns {Promise<string>} - 生成的图片URL
 */
export const generateSceneImage = async (scene, referenceImages = [], userId = null, organizationId = null, previousSceneImage = null, stylePrompt = null) => {
  const baseStyle = stylePrompt || '统一的艺术风格，连贯的视觉风格，相同的画风';
  const scenePrompt = `${scene.content}，${scene.shotType}，${scene.cameraMovement}，${baseStyle}，高质量，电影感，风格一致`;
    ...
    if (referenceImage) {
      ...
      response = await fetch(`${API_BASE_URL}/api/ai/image-to-image`, {
        ...
        body: JSON.stringify({
          ...
          user_id: userId,
          organization_id: organizationId,
          strength: previousSceneImage ? 0.6 : 0.5
        })
      });
    } else {
      response = await fetch(`${API_BASE_URL}/api/ai/generate-images`, {
