 ...
 * @param {string} userId - 用户ID
 * @param {string} organizationId - 组织ID
 * @param {string} previousSceneImage - 上一个分镜的图片URL（用于风格一致性）
...
export const generateSceneImage = async (scene, referenceImages = [], userId = null, organizationId = null, previousSceneImage = null, stylePrompt = null) => {
  ...
  try {
    ...
    // 选择参考图片：优先使用上一个分镜的图片（保持风格一致），否则使用用户上传的参考图片
    let referenceImage = previousSceneImage;
    if (!referenceImage && referenceImages && referenceImages.length > 0) {
