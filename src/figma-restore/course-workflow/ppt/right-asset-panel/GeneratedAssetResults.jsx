...
export function GeneratedAssetResults({ kind, asset, selectedIndex, onSelect, onRegenerate, onInsert }) {
  ...
  return (
    <div className="ppt-result-panel">
      <div className="ppt-result-desc">{kind === 'image' ? '选择候选中 · 加载完成' : '选择候选结果 · 已自动保存到素材库'}</div>
      <div className={`ppt-result-grid kind-${kind}`}>
        {cards.map((label, index) => (
          ...
          >
            <span className="ppt-result-thumb">
              ...
              {selectedIndex === index ? <i>✓</i> : null}
              {kind === 'image' ? <small><RefreshCw size={12} /></small> : null}
            </span>
      ...
      {kind === 'image' ? (
        <button type="button" className="ppt-result-regen" onClick={onRegenerate}><RefreshCw size={13} />重新生成</button>
      ) : null}
