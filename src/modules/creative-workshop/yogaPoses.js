/**
 * 体式库常量（与后端 api/src/lib/experience/yogaPoses.ts 同源，改动需两处同步）
 * 来源：newpage/体式-身体反应-情境匹配库.md
 * - en：唯一英文全名（注入图片 prompt）
 * - label：图上标注短名
 */
...
export const YOGA_PAGE_TYPES = [
  { value: 'scene', label: '场景页' },
  { value: 'transition', label: '过渡页' },
  { value: 'action', label: '动作页' },
  { value: 'return', label: '回归页' },
  { value: 'ending', label: '结束页' },
];

// 按英文全名查体式（生图时取身体反应描述）
export const findYogaPose = (en) => {
  const name = String(en || '').trim().toLowerCase();
  if (!name) return undefined;
  return YOGA_POSES.find((p) => p.en.toLowerCase() === name)
