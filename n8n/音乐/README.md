...
## 2026-09-18 线上 ComfyUI 地址

线上工作流 `dSzHC0vfUXO3GgTo` 的 ComfyUI 地址暂时固定为 `http://117.50.214.226:8188`；此前短暂加入的主备探测节点已移除。整曲提交、history 查询、音频下载及转写均沿用配置节点的这一地址。变更脚本为 `n8n/音乐/set-music-comfy-url.mjs`（只适用于此次变更前的流程状态，不要重复运行）。此目录的旧 JSON 导出不是 9 月 18 日线上最新版本，勿直接导入覆盖线上工作流。
