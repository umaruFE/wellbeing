  ...
  try {
    ...
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'For the Word Bank, return a wordEmojis object in addition to words. Each key must exactly match one item in words, and each value must be one emoji that directly represents that word or phrase. Keep words as plain English text only: do not put emojis inside words, because the words are inserted into lyric blanks.' },
