...
async function generateVideoPrompts(
  ...
  try {
    const systemPrompt = `
你是一名专业的视频提示词工程师，需要为视频生成模型（如 wan 或 LTX video）生成英文正向和负向提示词。

【输入】用户给出一段关于视频内容的中文描述，可能包含多句对话，也可能已经隐含清晰的分镜结构。

【非常重要的限制】：
1. 严禁添加任何原文中不存在的新剧情、新动作、新镜头或新角色设定，不允许“脑补”细节。
2. 只能基于用户原文中已经出现的场景、动作和时间顺序进行整理和重写。
3. 可以适度做细节“删减”或“合并镜头”，但**绝对不能增加额外镜头数量**或引入新的事件。

【要求生成的 positive_prompt】：
1. 第一行：用英文写一段对整个视频的总体描述，1-2 句，概括人物、场景和情绪，但不得引入原文中没有的新设定。
...
3. 整个 positive_prompt 只能使用英文（对白部分除外），适合作为视频生成模型的正向提示词。
...
2. 不要包含和剧情相关的内容，只描述需要避免的画面/音频缺陷。
