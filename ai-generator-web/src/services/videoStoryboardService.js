...
/**
 * 通过后端生成分镜脚本，避免在浏览器中暴露大模型密钥和系统提示词。
 * @param {string} description - 视频描述
...
export const generateStoryboardScript = async (description, referenceImages = [], duration = 30) => {
  try {
    const response = await fetch('/api/ai/generate-storyboard-script', {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({
        description,
    ...
    return {
      title: '默认分镜脚本',
      scenes: [
        { sequence: 1, duration: '0-3s', shotType: '中景', cameraMovement: '固定镜头', content: '开场画面', narration: '', generatedImage: null },
        { sequence: 2, duration: '3-6s', shotType: '近景', cameraMovement: '缓慢推镜', content: '主要内容', narration: '', generatedImage: null },
        { sequence: 3, duration: '6-9s', shotType: '全景', cameraMovement: '拉远', content: '结尾画面', narration: '', generatedImage: null }
      ]
    };
  }
};
